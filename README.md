...
## Latest CI-validated results
The latest successful GitHub Actions run executes the unit tests, real-data backtest and notebook from end to end. On the evaluated walk-forward sample, the Ridge strategy produced:

| Metric | Ridge ML | Momentum baseline | SPY* |
|---|---:|---:|---:|
| Annualized return | **3.92%** | -5.28% | 13.12% |
| Annualized volatility | **6.68%** | 7.00% | 15.53% |
| Sharpe ratio | **0.59** | -0.75 | 0.85 |
| Max drawdown | **-8.86%** | -26.42% | -23.93% |
| Cumulative return | **22.34%** | -24.80% | 91.04% |

For the separate **2024-present** diagnostic, the Ridge strategy returned **5.78% annualized** with a **0.81 Sharpe** and **-7.26% max drawdown**. Mean monthly cross-sectional rank IC over the full walk-forward experiment was **0.065**.

\*SPY is included for market context, not as an apples-to-apples benchmark: the research portfolio is dollar-neutral and much lower volatility.

Full machine-readable outputs are committed under `results/` and regenerated by CI.
...
- `results/` — validated metrics and robustness outputs.
